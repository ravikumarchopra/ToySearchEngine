...
class ESA():
    ...
    def __init__(self):
        ...
        print('Initializing ESA-LSA based IR system :')
    ...
    def buildIndex(self, docs, docIDs, articles, articleIDs):
        ...
        print('Performing SVD on document article-matrix ...')
        # Performing SVD
        k = np.linalg.matrix_rank(self.doc_art_matrix)
        U, S, V = np.linalg.svd(self.docVectors, full_matrices=True)
        U_k, S_k, V_k = U[:, :k], np.diag(S[:k]), V[:k]
        rank_k_approximation = np.dot(U_k, np.dot(S_k, V_k))
        self.docVectors = rank_k_approximation
        print('[ SVD Performed. ]')
    ...
    def rank(self, queries):
        ...
        print('Performing SVD on query-article matrix ...')
        # Performing SVD
        k = np.linalg.matrix_rank(query_art_matrix)
        U, S, V = np.linalg.svd(query_art_matrix, full_matrices=True)
        U_k, S_k, V_k = U[:, :k], np.diag(S[:k]), V[:k]
        rank_k_approximation = np.dot(U_k, np.dot(S_k, V_k))
        query_art_matrix = rank_k_approximation
        print('[ SVD Performed. ]')
